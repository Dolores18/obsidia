...
impl ApiManager {
    ...
    fn get_api_round_robin(&mut self, model: &str) -> Option<&Box<dyn ApiConfig>> {
    ...
    fn get_api_random(&self, model: &str) -> Option<&Box<dyn ApiConfig>> {
        self.configs.get(model).and_then(|apis| apis.choose(&mut rand::thread_rng()))
    }

    fn get_claude_api(&mut self, use_random: bool) -> Option<&Box<dyn ApiConfig>> {
        if use_random {
            self.get_api_random("claude")
        } else {
            self.get_api_round_robin("claude")
        }
    }

    fn get_gpt_api(&mut self, use_random: bool) -> Option<&Box<dyn ApiConfig>> {
        if use_random {
            self.get_api_random("gpt")
        } else {
            self.get_api_round_robin("gpt")
        }
    }
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // 使用 Claude API (轮询)
    for _ in 0..3 {
        if let Some(claude_api) = api_manager.get_claude_api(false) {
            println!("Using Claude API (Round Robin):");
    ...
    // 使用 Claude API (随机)
    for _ in 0..3 {
        if let Some(claude_api) = api_manager.get_claude_api(true) {
            println!("Using Claude API (Random):");
            process_config(&**claude_api);
        }
    }
    ...
    // 使用 GPT API (轮询)
    for _ in 0..3 {
        if let Some(gpt_api) = api_manager.get_gpt_api(false) {
            println!("Using GPT API (Round Robin):");
    ...
    // 使用 GPT API (随机)
    for _ in 0..3 {
        if let Some(gpt_api) = api_manager.get_gpt_api(true) {
            println!("Using GPT API (Random):");
            process_config(&**gpt_api);
        }
    }
