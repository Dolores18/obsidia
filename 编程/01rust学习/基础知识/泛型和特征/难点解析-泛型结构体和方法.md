...
下面是结构体方法的实现
   impl<Inner: AeadCore, const SALT_LEN: usize> RustCryptoAead<Inner, SALT_LEN> {
       fn recreate_crypto(&mut self, key: &[u8; Self::KEY_LEN], iv: &[u8; Self::IV_LEN]) {
           let mut subkey = [0u8; Self::KEY_LEN];
           Hkdf::<Sha1>::new(Some(iv), key)
               .expand(b"ss-subkey", &mut subkey)
               .unwrap();

           self.inner = Inner::new_from_slice(&subkey).unwrap();
...
let rust_crypto_aead =  RustCryptoAead<Aes128Gcm, 16>::new();
let aead = create_crypto(&key, &iv);
